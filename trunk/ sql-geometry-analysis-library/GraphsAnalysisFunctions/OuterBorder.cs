...
public partial class UserDefinedFunctions
{
    ...
    {
        SqlGeometry lattice = new SqlGeometry();
        ...
        for (int i = 1; i <= dotQty; i++)
        {
            ...
            lattice = lattice.STUnion(intersection);
        }
        int polygonsQty = (int)lattice.STNumGeometries();
        double[] xValues = new double[polygonsQty];
        int[] orderNumbers = new int[polygonsQty];
        for (int i = 1; i <= polygonsQty; i++)
        {
            xValues[i - 1] = lattice.STGeometryN(i).STPointN(1).STX.Value;
            orderNumbers[i - 1] = i;
        }
        ...
        {
            SqlGeometry currentPolygon = lattice.STGeometryN(orderNumbers[i]);
            SqlGeometry nextPolygon = lattice.STGeometryN(orderNumbers[i + 1]);
